...
class WallAround(unittest.TestCase):
    ...
    def test_io(self):
        left, right = self.set_and_get(400, 0, 0, 200) #total: 600
        self.assertTrue(left > right, "can't curve to right")

        left, right = self.set_and_get(0, 5, 1000, 0) #curve to left
        ...
        self.assertTrue(0 < left and 0 < right, "curve wrongly")

if __name__ == '__main__':
    time.sleep(3)
    rospy.init_node('travis_test_wall_around')
    rostest.rosrun('pimouse_run_corridor', 'travis_test_wall_around', WallAround)
